...
export function getAllUserIds(): number[] {
  return (
    db.prepare('SELECT id FROM users').all() as any[]
  ).map(r => r.id);
}
...
export function updateOrder(
  ...
  if (patch.receipt_file_id) {
    fields.push('receipt_file_id=?');
    vals.push(patch.receipt_file_id);
  }

  if (patch.admin_comment) {
    fields.push('admin_comment=?');
    vals.push(patch.admin_comment);
  }
...
export function getReferralEarnings(inviterId: number): number {
  const r: any = db.prepare(
    'SELECT SUM(cashback) as t FROM referrals WHERE inviter_id=?'
  ).get(inviterId);

  return r?.t ?? 0;
}

export function processCashback(
  inviteeId: number,
  orderAmount: number
): void {
  ...
  db.prepare(
    'UPDATE referrals SET cashback=cashback+? WHERE invitee_id=?'
  ).run(cashback, inviteeId);
