...
import {
  ...
  subtractBalance, addBalance, getAllUserIds, getUserCount,
...
const WEBAPP_URL  = process.env.WEBAPP_URL ?? `https://${process.env.RENDER_EXTERNAL_HOSTNAME ?? 'localhost:3000'}`;
...
const T: Record<Lang, any> = {
  ru: {
    ...
    enterUsername: '📝 Введите @username получателя (или свой):',
    invalidUser:   '❌ Неверный username. Используйте только буквы, цифры и _ (мин. 3 символа):',
  ...
  uz: {
    ...
    enterUsername: "📝 Qabul qiluvchining @username ni kiriting (yoki o'zingiznikini):",
    invalidUser:   "❌ Noto'g'ri username. Faqat harf, raqam va _ ishlating (min. 3 belgi):",
...
const adminRejectMap = new Map<number, string>();

function getState(uid: number): State { return states.get(uid) ?? { step: 'idle' }; }
function setState(uid: number, s: Partial<State>): void {
  states.set(uid, { ...getState(uid), ...s });
}
...
async function startBot() {
  ...
  bot.onText(/\/broadcast (.+)/, async (msg, match) => {
    ...
    await bot.sendMessage(ADMIN_ID, `✅ Рассылка:\n✅ ${sent} доставлено\n❌ ${failed} ошибок`);
  });

  bot.onText(/\/stats/, async (msg) => {
    if (msg.from!.id !== ADMIN_ID) return;
    await bot.sendMessage(ADMIN_ID,
      `📊 *Статистика*\n\n👥 Пользователей: *${getUserCount()}*\n🔍 На проверке: *${getPendingOrders().length}*`,
      { parse_mode: 'Markdown' }
    );
  });
  ...
  bot.onText(/\/addbalance (\d+) (\d+)/, async (msg, match) => {
    ...
    await bot.sendMessage(ADMIN_ID, `✅ Добавлено ${fmt(amount)} сум пользователю ${uid}`);
  });
  ...
  bot.on('callback_query', async (q) => {
    ...
    if (data.startsWith('approve:') && uid === ADMIN_ID) {
      ...
      const ul = getLang(order.user_id);
      try {
    ...
    if (data.startsWith('reject:') && uid === ADMIN_ID) {
      ...
      adminRejectMap.set(uid, orderId);
      await rmKb();
  ...
  bot.on('message', async (msg) => {
    ...
    // Admin reject reason
    if (uid === ADMIN_ID) {
      const rejectId = adminRejectMap.get(uid);
      if (rejectId && text && !text.startsWith('/')) {
        adminRejectMap.delete(uid);
    ...
    if (state.step === 'awaiting_username' && text && !text.startsWith('/')) {
      ...
      // Allow 3+ chars, letters/digits/underscore
