...
async function startBot() {
  ...
  bot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
    ...
    if (match?.[1] === 'topup') {
      ...
      await bot.sendMessage(uid, `💰 *Пополнение баланса*\n\nКарта: \`${CARD_NUMBER}\`\nВладелец: *${CARD_HOLDER}*\n\n📝 Введите сумму:`, { parse_mode: 'Markdown', reply_markup: { remove_keyboard: true } });
      return;
    }
  ...
  bot.onText(/\/support/, async (msg) => { await bot.sendMessage(msg.from!.id, tr(getLang(msg.from!.id) as Lang, 'supportText'), { parse_mode: 'Markdown' }); });
  bot.onText(/\/stats/, async (msg) => {
    if (msg.from!.id !== ADMIN_ID) return;
    const stats = getOrderStats();
    const users = getUserCount();
    const pending = getPendingOrders().length;
    await bot.sendMessage(ADMIN_ID, `📊 *Статистика*\n\n👥 Пользователей: *${users}*\n🔍 На проверке: *${pending}*\n\n📅 Сегодня: ${stats.today.cnt ?? 0} / ${fmt(stats.today.total ?? 0)} сум\n📅 Неделя: ${stats.week.cnt ?? 0} / ${fmt(stats.week.total ?? 0)} сум\n📅 Месяц: ${stats.month.cnt ?? 0} / ${fmt(stats.month.total ?? 0)} сум`, { parse_mode: 'Markdown' });
  });
  bot.onText(/\/broadcast (.+)/, async (msg, match) => {
    ...
    for (const id of ids) {
      try { await bot.sendMessage(id, text, { parse_mode: 'Markdown' }); sent++; } catch { failed++; }
  ...
  bot.onText(/\/dm(?:\s+(.+))?/, async (msg, match) => {
    ...
    try {
      ...
      await bot.sendMessage(ADMIN_ID, `✅ Сообщение отправлено \`${targetId}\`.`, { parse_mode: 'Markdown' });
  ...
  bot.on('callback_query', async (q) => {
    ...
    if (data === 'pay:balance') {
      const state = getState(uid);
      if (!subtractBalance(uid, state.price!)) {
    ...
    if (data.startsWith('reject:') && uid === ADMIN_ID) {
      adminReject.set(uid, data.replace('reject:', ''));
      await rmKbFn(q.message!.chat.id, q.message!.message_id);
  ...
  bot.on('message', async (msg) => {
    ...
    if (uid === ADMIN_ID) {
      ...
      if (rejectId && rejectId.startsWith('topup')) {
        ...
        const targetUserId = parseInt(rejectId.split(':')[1], 10);
        addBalance(targetUserId, amount);
        updateOrder(rejectId, { status: 'approved', admin_comment: String(amount) });
      ...
      if (state.step === 'awaiting_dm_text' && text && !text.startsWith('/')) {
        ...
        try {
          ...
          await bot.sendMessage(uid, `❌ Не удалось доставить. Пользователь \`${targetId}\` заблокировал бота.`, { parse_mode: 'Markdown', reply_markup: { remove_keyboard: true } });
        }
        return;
      }
          ...
          if (order) {
            ...
            try {
              await bot.sendMessage(order.user_id,
                tr(ul, 'rejected', { product: order.product_name, reason: reason }),
    ...
    if (state.step === 'awaiting_promo' && text && !text.startsWith('/')) {
      ...
      if (res.ok) {
        await bot.sendMessage(uid, tr(lang, 'promoOk', { bonus: fmt(res.bonus!) }), { parse_mode: 'Markdown' });
      } else {
        await bot.sendMessage(uid, tr(lang, 'promoFail'), { parse_mode: 'Markdown' });
      }
      setState(uid, { step: 'main_menu' });
      await bot.sendMessage(uid, 'Главное меню', { reply_markup: mainKb(lang) });
      return;
    }
    ...
    if (text === tr(lang, 'referral')) {
      ...
      await bot.sendMessage(uid,
        tr(lang, 'refText', { link, count: String(getReferralCount(uid)), earned: fmt(getReferralEarnings(uid)) }),
        { parse_mode: 'Markdown' }
      );
      return;
    }
    ...
    if (text === tr(lang, 'promoCmd')) {
      ...
      await bot.sendMessage(uid, tr(lang, 'promoAsk'), { reply_markup: cancelKb() });
      return;
    }
